...
if ($po['status'] !== 'draft') {
    $_SESSION['po_flash_error'] = __('po_err_not_draft');
...
$formSupplierId = (string) $po['supplier_id'];
$formOrderDate = $po['order_date'];
$formExpectedDate = $po['expected_date'] ?? '';
$formNote = $po['note'] ?? '';

$itemStmt = $pdo->prepare('SELECT * FROM purchase_order_items WHERE purchase_order_id = ? ORDER BY id');
$itemStmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $_SESSION['po_flash'] = __('po_updated_toast');
        header('Location: ' . BASE_URL . '/purchase-order/view.php?id=' . $id);
        exit;
...
<h4 class="mb-4"><?= __('po_edit_title') ?> — <span class="mono"><?= htmlspecialchars($po['reference']) ?></span></h4>
...
<form method="post" id="poForm">
  ...
  <div class="card p-3">
    ...
    <button id="poSubmitButton" class="btn btn-primary w-100 mt-2"><i class="bi bi-save"></i> <?= __('common_save') ?></button>
  </div>
</form>

<script>
const PRODUCTS = <?= json_encode($products) ?>;
const EXISTING_ITEMS = <?= json_encode(array_map(fn($it) => ['product_id' => (int) $it['product_id'], 'ordered_qty' => (int) $it['ordered_qty'], 'unit_cost' => (float) $it['unit_cost']], $existingItems)) ?>;
...
if (EXISTING_ITEMS.length) {
  EXISTING_ITEMS.forEach(it => addRow(it.product_id, it.ordered_qty, it.unit_cost.toFixed(2)));
} else {
  addRow();
}
</script>
